...
import { useDeputyCountyManagerList } from 'src/fetch/county_managers';
...
const DeputyCountyManagerList = () => {
  ...
  const { data, isLoading, error, isSuccess } = useDeputyCountyManagerList();

  if (error) {
    console.log(`Error => ${error}`);
    enqueueSnackbar('Error fetching deputy county managers', {
      variant: 'error'
    });
  }
        ...
        <Box className={classes.actionItem}>
          <Tooltip title="View" placement="bottom">
            <Avatar className={clsx(classes.dark, classes.viewAction)}>
              <ArrowRight
                onClick={() =>
                  navigate(`/app/deputy_county_managers/${params.value.id}`)
                }
              />
            </Avatar>
          </Tooltip>
  ...
  return (
    <Page title={`${siteNames?.DCM?.name}s`} className={classes.root}>
      <div className={classes.progress}>{isLoading && <LineProgress />}</div>
      <Container maxWidth={false}>
        <PageToolbar title={`${siteNames.DCM?.name}s`} />
        <DataGridDisplay
          data={countyManagerData}
          title={`${siteNames.DCM?.name}s`}
        />
      </Container>
    </Page>
  );
...
export default DeputyCountyManagerList;
