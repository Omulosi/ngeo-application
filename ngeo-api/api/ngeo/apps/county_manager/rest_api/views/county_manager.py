...
from ..serializers import CountyManagerSerializer, DeputyCountyManagerSerializer
...
class DeputyCountyManagerList(generics.ListAPIView):
    """
    Returns a list of deputy county managers for a 
    given user.
    Returns all deputy county managers if the user has role HR.
    """

    serializer_class = DeputyCountyManagerSerializer
    queryset = DeputyCountyManager.objects.all()
    ...
    def get_queryset(self):
        ...
        # All DCMs in this region
        if user.role == User.RM:
            region_manager = get_object_or_404(RegionalManager, user=user)
            region = region_manager.area.region
            if region:
                return self.queryset.filter(Q(area__region=region) & Q(user__role=User.DCM))
            return self.queryset.filter(Q(regional_manager=region_manager) & Q(user__role=User.DCM))
        # return self.queryset.filter(user__role=User.CM)
        return self.queryset



class DeputyCountyManagerDetail(generics.RetrieveDestroyAPIView):
    """
    Returns one deputy county manager
    """
